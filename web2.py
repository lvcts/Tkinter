...
def __entry():
    if entry1.get() == "" or entry2.get() == "" or entry3.get() == "":
        ...
        borndate = spinbox1.get()
        bornmonth = spinbox2.get()
        bornyear = spinbox3.get()
        address = entry4.get()
        #===================================
        sql = "INSERT INTO user (firstname, lastname, email, gender, university, borndate, bornmonth, bornyear, address) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
        value = [(firstname),
                 (lastname),
                 (email),
                 (gender),
        ...
        textarea.insert(END,f"\n\n Born Date:\t\t{borndate}-{bornmonth}-{bornyear}")
        textarea.insert(END,f"\n\n Address:\t\t{address}")
        #textarea.insert(END,f"\n\n {checkbutton.get()}")
        return
